...
function setupAuthHandlers() {
  ...
  registerForm.onsubmit = async (e) => {
    ...
    try {
      ...
      switchBtn.click(); // Switch back to login form
  ...
  switch (tabName) {
    ...
    case 'reports':
      ...
      document.querySelector('.btn-ws-dl-pl').onclick = () => {
        ...
        window.open(`/api/ca/reports/pdf?clientId=${clientId}&reportType=pl&caId=${caSession.id}`, '_blank');
      };
      document.querySelector('.btn-ws-dl-bs').onclick = () => {
        const caSession = getCASession();
        if (!caSession) return;
        showToast('Generating GST Return Summary PDF...');
        window.open(`/api/ca/reports/pdf?clientId=${clientId}&reportType=gst&caId=${caSession.id}`, '_blank');
      };
...
function initCommandBar() {
  ...
  function renderList(query) {
    ...
    let filteredSuggestions = commandSuggestions.filter(s => s.text.toLowerCase().includes(q));
      ...
      filteredClients.forEach(c => {
        ...
        html += `
          <div class="command-result-item" data-item-id="${id}">
            ...
            <span>Open workspace for <strong>${c.name}</strong> (${c.owner})</span>
          </div>
        `;
      });
    }
    ...
    // Set up initial selection highlight
    const items = commandResults.querySelectorAll('.command-result-item[data-item-id], #ai-command-search-btn');
...
// --------------------------------------------------------------------------
// 9. Interactive Toast & App Bindings
// --------------------------------------------------------------------------
...
// ==========================================================================
// 6. Analytics Charts Logics (Phase 6)
// ==========================================================================
...
// --------------------------------------------------------------------------
// Real-time Chart.js Theme Swapping Helper
// --------------------------------------------------------------------------
...
// --------------------------------------------------------------------------
// Notifications System (bell icon, dropdown menu tray)
// --------------------------------------------------------------------------
  ...
  globalNotifications = [
    {
      ...
      desc: `Mismatched GSTR-2B purchase invoices for ${acme ? acme.name : 'Acme Corp'} (₹45,200).`,
      time: '10m ago',
      unread: true,
      action: () => {
        if (acme) {
...
function parseMarkdown(text) {
  ...
  html = html.replace(/```([\s\S]*?)```/g, '<pre style="background: var(--bg-app); border: 1px solid var(--border); padding: var(--space-8); border-radius: var(--radius-6); overflow-x: auto; font-family: monospace; font-size: 13px; margin: var(--space-8) 0;"><code>$1</code></pre>');

  // Inline code (single backtick)
  html = html.replace(/`([^`]+)`/g, '<code style="background: var(--bg-app); border: 1px solid var(--border); padding: 2px 4px; border-radius: var(--radius-4); font-family: monospace; font-size: 13px;">$1</code>');
  ...
  const processedLines = lines.map(line => {
    ...
    if (line.startsWith('<li') || line.startsWith('<h') || line.startsWith('<pre') || line.startsWith('<code') || line.trim() === '') {
      return line;
    }
    return line + '<br>';
  });
