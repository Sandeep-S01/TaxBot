import { Client, GstRate } from '../types';
...
The response must conform exactly to this schema:
{
  ...
  "transcription": "string (the raw transcription of what the user said)"
    ...
    if (extraction.error) {
      if (extraction.error === 'not_a_transaction') {
        const prompt = `You are a helpful Indian GST-compliant accounting assistant. The user sent a voice message that was transcribed as: "${extraction.transcription || 'Hi/Hello'}"
Respond politely in a single, helpful sentence.`;
        const reply = await askGemini(prompt, extraction.transcription || 'hello', 'en');
    ...
    const gstRate = extraction.gst_rate !== undefined ? (extraction.gst_rate as GstRate) : 0;
    const gstCategory = extraction.gst_category || (extraction.category === 'sales' ? 'B2C' : 'exempt');
    ...
    const tx = await createTransaction({
      ...
      raw_text: extraction.transcription || 'Voice message',
