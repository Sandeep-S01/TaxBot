...
export async function handleInteractive(
  ...
  // 1. Handle review actions: review_<txId>_<confirm|reject>
  if (replyId.startsWith('review_')) {
    const parts = replyId.split('_');
    if (parts.length < 3) {
      await sendMessage(client.phone, '⚠️ Invalid review action.');
      return;
    }
    ...
    try {
      ...
      await sendMessage(client.phone, '⚠️ Unknown review action.');
    } catch (err: any) {
      console.error('Error handling interactive review action:', err.message);
      await sendMessage(client.phone, '⚠️ Failed to update transaction review status. Please try again.');
    }
    return;
  }

  // 2. Handle category changes: cat_<txId>_<category>
  ...
  // 3. Handle export format selection: exp_<period>_<format>
