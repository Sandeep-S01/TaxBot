...
import { getTransactionById, updateTransactionCategory, updateTransactionStatus } from '../db/transactions';
...
import { isValidTransactionCategory, parseExportReplyId } from './interactiveValidation';
...
export async function handleInteractive(
  ...
  if (replyId.startsWith('review_')) {
    const parts = replyId.split('_');
    if (parts.length !== 3) {
      await sendMessage(client.phone, 'Invalid review action.');
      return;
    }
    ...
    try {
      ...
      if (!tx || tx.client_id !== client.id) {
        await sendMessage(client.phone, 'Transaction not found or access denied.');
        return;
      }

      if (action === 'confirm') {
        const updated = await updateTransactionStatus(txId, 'confirmed', null);
        await sendMessage(
          client.phone,
          `*Transaction Confirmed!*\n\n- *Transaction:* ${updated.vendor_name || 'Entry'}\n- *Amount:* ${formatINR(updated.amount + updated.tax_amount)}\n\nThis entry is now included in your reports and GST summaries.`
        );
        return;
      }

      if (action === 'reject') {
        const updated = await updateTransactionStatus(txId, 'rejected', 'rejected_by_user');
        await sendMessage(
          client.phone,
          `*Transaction Rejected!*\n\n- *Transaction:* ${updated.vendor_name || 'Entry'}\n- *Amount:* ${formatINR(updated.amount + updated.tax_amount)}\n\nThis entry will stay out of your reports.`
        );
        return;
      }

      await sendMessage(client.phone, 'Unknown review action.');
    } catch (err: any) {
      console.error('Error handling interactive review action:', err.message);
      await sendMessage(client.phone, 'Failed to update transaction review status. Please try again.');
    }
    return;
  }
  ...
  if (replyId.startsWith('cat_')) {
    const parts = replyId.split('_');
    if (parts.length !== 3) {
      await sendMessage(client.phone, 'Invalid category change request.');
      return;
    }

    const txId = parts[1];
    const newCategory = parts[2];
    if (!isValidTransactionCategory(newCategory)) {
      await sendMessage(client.phone, 'Invalid category change request.');
      return;
    }
    ...
    try {
      ...
      await sendMessage(client.phone, 'Failed to update transaction category. Please try again.');
    }
    return;
  }

  if (replyId.startsWith('exp_')) {
    const exportRequest = parseExportReplyId(replyId);
    if (!exportRequest) {
      await sendMessage(client.phone, 'Invalid export request.');
      return;
    }

    const { period, format } = exportRequest;

    try {
      const todayStr = new Date().toISOString().split('T')[0];
      const token = generateExportToken(client.id, period, todayStr);
      const host = getPublicAppOrigin();
      ...
      const msg = `*Your ${formatLabel} Link is Ready!*\n\n` +
      ...
      await sendMessage(client.phone, 'Failed to generate export link. Please try again.');
    }
    return;
  }
