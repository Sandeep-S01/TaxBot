...
export async function handleDocument(
  ...
  try {
    ...
    // 3. Extract text contents using pdf-parse
    let parsedText = '';
    try {
      const pdfData = await pdf(downloaded.buffer);
    ...
    // 4. Check for scanned PDFs (which have little to no selectable text)
    ...
    // 5. Send text to Claude to extract transactions
    ...
    // 6. Save extracted transactions to database
    for (const rawTx of extraction.transactions) {
      ...
      try {
        const tx = await createTransaction({
          ...
          source: 'whatsapp_pdf',
          raw_text: `PDF OCR Row: ${JSON.stringify(rawTx)}`,
    ...
    console.error('Error handling PDF document:', error);
    await sendMessage(
      client.phone,
      '❌ Sorry, I had trouble parsing that PDF document. Please verify the file is not password-protected and upload it again.'
    );
  }
}
