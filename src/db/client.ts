...
if (!supabaseUrl || !supabaseKey) {
  if (process.env.NODE_ENV === 'production') {
    throw new Error('SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY must be defined in production.');
  }
  console.warn(
...
if (process.env.SUPABASE_SERVICE_ROLE_KEY === 'your_supabase_service_role_secret_key') {
  if (process.env.NODE_ENV === 'production') {
    throw new Error('SUPABASE_SERVICE_ROLE_KEY is still set to the default placeholder.');
  }
  console.warn(
