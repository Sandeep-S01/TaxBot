...
import { normalizeExtraction } from './validateExtraction';
 ...
 * Automatically chooses Gemini 2.5 Flash if GEMINI_API_KEY is available,
...
export async function categoriseReceipt(
  ...
  try {
    ...
    // If Gemini key is present, use Gemini 2.5 Flash.
    if (GEMINI_API_KEY) {
...
---`,
    ...
    if (result.transactions && Array.isArray(result.transactions)) {
      result.transactions = result.transactions.map((tx) => normalizeExtraction(tx));
    }
